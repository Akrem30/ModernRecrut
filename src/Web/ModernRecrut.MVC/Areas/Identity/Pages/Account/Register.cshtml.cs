...
namespace ModernRecrut.MVC.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public class InputModel
        {
            ...
            [Compare("Password", ErrorMessage = "La confiramtaion du mot de passe a échoué")]
            ...
            [Required(ErrorMessage ="Choissisez un type")]
            [DisplayName("Type")]
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (user.Type == TypeUtilisateur.Employé)
                    {
                        var resultat = await _userManager.AddToRoleAsync(user, TypeUtilisateur.Employé.ToString());

                        if (resultat.Succeeded)
                            _logger.LogInformation($"Le rôle {TypeUtilisateur.Employé.ToString()} a été attribué au nouvel utilisateur avec id {user.Id}.");
                    }
                    else
                    {
                        var resultat = await _userManager.AddToRoleAsync(user, TypeUtilisateur.Candidat.ToString());

                        if (resultat.Succeeded)
                            _logger.LogInformation($"Le rôle {TypeUtilisateur.Candidat.ToString()} a été attribué au nouvel utilisateur avec id {user.Id}.");
                    }
